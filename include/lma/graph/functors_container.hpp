...
namespace lma
{
  ...
  template<class L> class FunctorsContainer
  {
    private:
      ...
      {
        using args_  = typename ttt::Extract<decltype(&F::operator())>::Args;
        using args   = typename boost::mpl::transform<args_,boost::add_pointer<boost::mpl::_1>>::type;
        using v_args = typename boost::fusion::result_of::as_vector<args>::type;
        using type   =  AlignedVector<std::pair<F,v_args>>;
      };

      using Types    = typename boost::mpl::transform<L, F2type<boost::mpl::_1> >::type;
      using Functors = typename boost::fusion::result_of::as_vector<Types>::type;
    ...
    public:
      ...
      {
        using Container = typename F2type<F>::type;
        using v_args    = typename F2type<F>::v_args;
